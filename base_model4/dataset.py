...
class HierarchicalInterviewDataset(Dataset):
    def __init__(self, data_path, max_turns=20, max_length=128):
    ...
    def __getitem__(self, idx):
        ...
        encoded_turns = [
            self.tokenizer(turn, padding='max_length', max_length=self.max_length, truncation=True, return_tensors="pt")
            for turn in dialogue_turns[:self.max_turns]
        ]

        while len(encoded_turns) < self.max_turns:
            encoded_turns.append(self.tokenizer("", padding='max_length', max_length=self.max_length, truncation=True, return_tensors="pt"))
