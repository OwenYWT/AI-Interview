...
class HongzhenAlbertForRegression(nn.Module):
    def __init__(self, model_name, num_outputs=3):
        ...
        hidden_size = self.albert.config.hidden_size  # Get the hidden size from AlbertModel
        
        self.relu1 = nn.ReLU()  # Corrected ReLU usage
        self.regressor = nn.Linear(hidden_size, num_outputs)  # Correctly using hidden_size

    def forward(self, input_ids, attention_mask, labels=None):
        # Forward pass through Albert
        outputs = self.albert(input_ids=input_ids, attention_mask=attention_mask)
        pooled_output = outputs[1]  # Use the pooled output (CLS token representation)
        
        # Apply activation and linear layer
        pooled_output = self.relu1(pooled_output)  # First activation
        pooled_output = self.regressor(pooled_output)  # Regression layer
        predictions = pooled_output  # Second activation
        ...
        # Return loss and predictions if labels are provided, else only predictions
