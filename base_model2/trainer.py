    ...
    def train(self):
        for epoch in range(self.max_epochs):
            ...
            for dialogue_turns, labels in tqdm(self.train_loader):
                dialogue_turns = {k: v.to(self.device) for k, v in dialogue_turns.items()}
                labels = labels.to(self.device)
                predictions = self.model(dialogue_turns)
        ...
        avg_mse = total_mse / len(self.train_loader.dataset)
        print(f"Epoch {epoch + 1} | Train Loss: {train_loss / len(self.train_loader):.4f} | Train MSE: {avg_mse:.4f}")
